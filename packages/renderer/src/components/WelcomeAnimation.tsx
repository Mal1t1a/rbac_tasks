  ...
  return (
    ...
    >
      <div className="text-center select-none">
        <h2
          className={`text-4xl md:text-5xl font-semibold tracking-tight transition-opacity duration-700 ${
            showHeader && !fadeOutText ? 'opacity-100' : 'opacity-0'
          } ${isLight ? 'text-black' : ''}`}
        >
          {`Welcome ${name}`}
        </h2>
        <p
          className={`mt-2 text-base md:text-lg transition-opacity duration-700 ${
            showSub && !fadeOutText ? 'opacity-100' : 'opacity-0'
